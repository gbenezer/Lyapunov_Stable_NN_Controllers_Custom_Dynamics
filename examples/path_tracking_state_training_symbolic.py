...
def main(cfg: DictConfig):
    ...
    dt = cfg.model.dt
    path_tracking_continuous = ss.PathTracking(speed=2.0, length=1.0, radius=10.0)

    dynamics = sd.GenericDiscreteTimeSystem(
        path_tracking_continuous,
        dt=dt,
        integration_method=sd.IntegrationMethod[cfg.model.integration],
    )
    ...
    if cfg.model.lyapunov.quadratic:
        Q = np.eye(2)
        R = np.eye(1)
        _, S = path_tracking_continuous.lqr_control(Q=Q, R=R)
        S_torch = torch.from_numpy(S).type(dtype).to(device)
